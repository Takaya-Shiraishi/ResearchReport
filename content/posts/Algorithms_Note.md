...
- 200SMA
  ...
  陰線なら200SMAが下にある場合、陽線なら200SMAが上にある場合にのみトレード。
- 平均足
  平均足の方向が200SMAの方向と一致しており、平均足の特性から
...
２．全ての時間足で平均足が陰線且つ200SMAがエントリー足より下ならば売り固定
３．全ての時間足で平均足が陽線且つ200SMAがエントリー足より上ならば買い固定
４．エントリー時のリスク対報酬比（RRR）が2以上であること。

## ストップロスの設定
エントリー足の前の平均足の高値または安値に、ATR（平均足の高値と安値の範囲の平均）の1.5倍を加えた（もしくは減じた）値をストップロスとして設定する。

## エントリー・利益確定方法
5分足の平均足にて全ての時間足の平均足と同じ色の線（陰線または陽線）が出たタイミングから毎本5分足の平均足を監視。トレンド方向と逆のひげが出なければその平均足でエントリー。

## 利益確定方法
利益確定ポイント1：エントリー時のリスク対報酬比（RRR）の2倍の価格変動に達した時点で、半分のポジションを利益確定。
利益確定ポイント2：エントリー時のリスク対報酬比（RRR）の4倍の価格変動に達した時点で、残りのポジションを利益確定。
また、利益確定ポイント1に達した場合、ストップロスをエントリーポイントに移動させ、利益が失われないようにする。


# 余談
> 禁忌的な手法

ChatGPTに為替データの機械学習を行うコード（学習に使用する指標等の特徴量はChatGPTが提案）を生成させ、そのコードで生成されたAIのバックテスト結果をChatGPTにインプットし、ChatGPTが改善した機械学習のコードを・・・ということを繰り返す手法。
これを２回ほど繰り返した結果、勝率は49%と50%を割っていますがロット張りのアルゴリズムはかなり最適化されたらしく2004年から現在までのデータで資金を２倍以上に増やすAIが作れました。
![](https://Takaya-Shiraishi.github.io/ResearchReport/img/2023-05-04-01-06-44.png)